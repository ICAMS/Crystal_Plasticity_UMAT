...
c================================================================
      module mod_mesh_grid_size
         implicit none
         integer, parameter :: Tnel=1
         integer, parameter :: Tngp=8
         integer, parameter :: Tnfx=2
         integer, parameter :: Tnfy=2
         integer, parameter :: Tnfz=2
         ...
         integer, parameter :: Iwkcoup_bk=0    !-->with(1), without(0)
...
c================================================================
      module mod_wkcoup_grad
         ...
         real(8), parameter :: C_taui    = 0.030    !-->Taylor hardening parameter
         real(8), parameter :: L_size    = 1.0d-9  !-->unit m, length scale
         real(8), parameter :: C_unit    = 1.0d-3  !-->unit m, mesh
...
c
         CD_parts(1)=3.d-15        ! inside  domain
...
c================================================================
     ...
     &              Rho_gnd, IVB_gnd, pk2i_gnd, tau_gnd)
            implicit none
...
c                  pvct1 = gvct*L_size - tvct*L_size/2*1.d10
c                  pvct2 = gvct*L_size + tvct*L_size/2*1.d10
                  bvct=0
                  do k=1,3
                     x1=0
                     do i1=1,3
...
c            real(8), parameter :: c_hard     = 5.d-3   !==> hardening coefficient of trip
c            real(8), parameter :: c_strs     = 1.0     !==> stress induced, unitless
c            real(8), parameter :: c_pben     = 0       !==> Eng of PB, MPa
c            real(8), parameter :: c_enuc     = 1.d-5   !==> strain induce nucleation
c            real(8), parameter :: c_pbmb0    = 1.0d-8  !==> fitting parameter speed
c            real(8), parameter :: B          = 5.0d0   !==> fitting parameter speed
c            real(8), parameter :: R          = 8.314d0 !==> gas constant   
...
cc-----------Shear band intersection induced martensite nucleation
c            do is=1,NTtrp
c               i=int(dabs(Cab(is,1)))
c               j=int(dabs(Cab(is,2)))
c               x1=dsign(1.d0, Cab(is,1))
c               x2=dsign(1.d0, Cab(is,2))
c               y1=dsign(1.d0, XVtwin(i))
c               y2=dsign(1.d0, XVtwin(j))
c               if(x1==y1.and.x2==y2)then
c                  x3=(dabs(XVtwin(i))+dabs(XVtwin(j)))/gam_crt**2
c               else
c                  x3=0
c               endif               
c               Vtrp_e(is)=Vtrp0_e(is)+c_nucl*(1-TVtrp0)*x3*dt1
c               if(Vtrp_e(is)<0) Vtrp_e(is)=0
c               if(Vtrp_e(is)>1) Vtrp_e(is)=1
c            enddo         
...
c            IVB_trp(1:Nslp) = c_hard * Xmu * TVtrp0**c_powr
c            IVB_trp(1:Nslp) = c_hard * Xmu * TVtrp0
c            IVB_trp(1:Nslp) = B_fcc*Xmu*dexp(8.8d0*TVtrp0)/(4.5d-6) 
...
c================================================================
         subroutine sub_int_evolution(epsx,epsy,epsz,epsp)
            implicit none
            integer i1,i2
            ...
            do i1=1,Tnel
            do i2=1,Tngp
            fem_intp(i1,i2,:)=sigR(1:6)
            fem_intx(i1,i2,:)=sigR(7:12)
            fem_inty(i1,i2,:)=sigR(13:18)
            fem_intz(i1,i2,:)=sigR(19:24)            
            enddo
            enddo
...
c================================================================
     ...
     &                          pk2i_intp)
         ...
         pk2i_intp(:)=fem_intp(iex,igx,:)
         pk2i_intx(:)=fem_intx(iex,igx,:)
         pk2i_inty(:)=fem_inty(iex,igx,:)
         pk2i_intz(:)=fem_intz(iex,igx,:)
...
c================================================================
      module mod_wkcoup_bk
         use mod_mesh_grid_size
         implicit none 
         real(8),parameter:: Adir = 2.0d3
         real(8),parameter:: Adyn = 10
         real(8),parameter:: N_slip = 12
...
c================================================================
         subroutine wkcoup_evolution(epsx,epsy,epsz,epsp,dtime)
            implicit none
            real(8) epsx(6),epsy(6),epsz(6),epsp(6),dtime
