...
c +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      if(lop==2)then
         ...
         call wkcoup_evolution(epsx,epsy,epsz,epsp,dtime)
      endif
...
c---------------------------------------------------------c
     ...
     &              refv_pk2i,refv_IVB,IVB_ini)
      if(time(2)==0)then
         ...
         !! Parameters controlling damage
         statev(152)=0                           !!--->152 Epl0 (eq. plastic strain)
      endif
...
c------------------------------------------------------------c
c    Damage variable calculation for ABAQUS element deletion c
c------------------------------------------------------------c
                     
      Epl = (matmul(transpose(Fp),Fp) - XI33)/2.0    ! Compute plastic strain
      Epl_inter = 0.0d0                              ! Initialize to 0.0
      
      !! Equivalent plastic strain calculation
      do i=1,3
      do j=1,3
      Epl_inter = Epl_inter + Epl(i,j)**2
      enddo
      enddo            
      Epl_eq = ((2.0/3.0)*Epl_inter)**0.5              
       
      Epl_min = 0.12                   	! Damage initiation plastic strain
      Epl_max = 0.17                    ! Complete failure plastic strain
      if(Epl_eq <= Epl_min)then
           statev(151) = 0.0d0
      else if(Epl_eq > Epl_max)then
           statev(151) = 0.99d0
      else
           statev(151) = statev(151) + ((Epl_eq-statev(152))/(Epl_max-Epl_min))
      endif
      
      !! Update the ABAQUS element damage status
      if(statev(150)==1)then                      
        if(statev(151) > 0.98d0)then
            statev(150) = 0
            statev(151) = 0.99d0
        endif                               
      endif
...
c------------------------------------------------c
      if(ising/=0)then
         ...
         do is=1,Nslp                         !!
            icomp=icomp+1                     !! 
            statev(icomp)=IVB(is)             !!-->41-52  IVB
         enddo                                !!
         
         do is=1,12                                  !!
            icomp=icomp+1; 
            statev(icomp)=IVB_gnd(is) !!-->53-64  Isotropic GND-hardening 
         enddo   

         do is=1,12                                  !!
            icomp=icomp+1; 
            statev(icomp)=tau_gnd(is) !!-->65-76  Kinematic GND-hardening 
         enddo    

         statev(150)=statev(150)
         statev(151)=statev(151)             !!--->151 Umat element damage value       
         statev(152)=Epl_eq                  !!--->152 Equivalent plastic strain
