...
c================================================================
c
c    material-specific subroutines with weak coupling to solution 
c    at Gauss points
c    Modules: mod_wkcoup_grad, mod_wkcoup_trip, mod_wkcoup_int,
c             mod_wkcoup_sup, mod_wkcoup
c             
c
c================================================================
...
c================================================================
      module mod_wkcoup_grad
         ...
         implicit none
c         integer, parameter :: I_gnd_iso = 1       !-->with(1),without(0)
c         integer, parameter :: I_gnd_kin = 1       !-->with(1),without(0)  
c         real(8), parameter :: C_taui    = 0.01    !-->Taylor hardening parameter
c         real(8), parameter :: L_size    = 1.0d-9  !-->unit m, length scale
c         real(8), parameter :: C_unit    = 1.0d-6  !-->unit m, mesh
c         real(8), parameter :: B_lattice = 2.5d-10 !-->unit m, Burgers vector 
c         real(8), parameter :: CD_smooth = 3.d-15  
c--------strain gradient
...
c================================================================
         subroutine mod_wkcp_ini(IB1,IB2)
            implicit none
            integer IB1(9),IB2(9)
            if(Iwkcoup_grad/=0) call sub_grad_ini
            if(Iwkcoup_trip/=0) call sub_trip_ini(IB1,IB2)
            if(Iwkcoup_int/=0)  call sub_int_ini
            if(Iwkcoup_sup/=0)  call sub_sup_ini
            return
         endsubroutine
c================================================================
