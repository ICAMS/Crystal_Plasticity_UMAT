...
""" 
 ...
 Homogenized values for total strain, plastic strain and stress
 ...
 $ abaqus python analyze_odb_structured_nopbc_J2PE".py {jobname}
    ...
    def F_ODB(self, odbName, firstStep, lastStep):
        ...
        for stepKey in range(firstStep, lastStep+1):
            ...
            for currentFrame in odb.steps[currentStepName].frames:
                ...
                epl1, epl2, epl3, epl4, epl5, epl6, test = self.average_elmts(
                    tens.getScalarField(componentLabel='PE11').values,
                    tens.getScalarField(componentLabel='PE22').values,
                    tens.getScalarField(componentLabel='PE33').values,
                    tens.getScalarField(componentLabel='PE12').values,
                    tens.getScalarField(componentLabel='PE13').values,
                    tens.getScalarField(componentLabel='PE23').values,
                ...
                self.testH.append(test)

                peeq_field = currentFrame.fieldOutputs['PEEQ']
                peeq_values = [val.data for val in peeq_field.values]
                avg_peeq = np.mean(peeq_values)
                print(avg_peeq)
                self.peeqH.append(avg_peeq)
...
header_str = '# time (s), total strain [11, 22, 33, 12, 13, 23] (.), plastic strain [11, 22, 33, 12, 13, 23] (.), stress [11, 22, 33, 12, 13, 23] (MPa), max. principal total strain (.), equiv. stress (MPa), peeq'
...
if lt > 0:
    ...
    out_val = zip(Result1.steptime,
              ...
              Result1.etotHE, Result1.sigmaHE, Result1.temperature, Result1.peeqH)
else:
    header_str += '\n'
    out_val = zip(Result1.steptime,
              Result1.etot1, Result1.etot2, Result1.etot3,
              ...
              Result1.sig1, Result1.sig2, Result1.sig3,
              Result1.sig4, Result1.sig5, Result1.sig6,
              Result1.etotHE, Result1.sigmaHE, Result1.peeqH)
