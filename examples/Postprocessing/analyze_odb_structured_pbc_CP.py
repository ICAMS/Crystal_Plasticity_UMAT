...
# -*- coding: utf-8 -*-
""" 
 Python script to analyse Abaqus OBD files generated under
 periodic boundary conditions
 
 Computes global stress and total strain from references nodes of periodic RVE
 
 Homogenized values for plastic strain
 are obtained via averaging of element values
 Different element sizes are not considered! 
 Use only for voxelized structures with structured mesh
 
 Evaluates plastic strains by SDV field output of Abaqus generated by ICAMS CP-UMAT
 
 Usage by Abaqus command line interface:
 $ abaqus python analyze_odb_structured_pbc_CP.py {jobname}
...
Authors:
 Martin Boeff, Ronak Shoghi, Alexander Hartmaier
    ...
    def F_ODB(self, odbName, dimension, firstStep, lastStep):
        ...
        for stepKey in range(firstStep, lastStep+1):
            ...
            for currentFrame in odb.steps[currentStepName].frames:
                ...
                epl1 = currentFrame.fieldOutputs['SDV156'].values
                epl2 = currentFrame.fieldOutputs['SDV157'].values
                epl3 = currentFrame.fieldOutputs['SDV158'].values
                epl4 = currentFrame.fieldOutputs['SDV159'].values
                epl5 = currentFrame.fieldOutputs['SDV160'].values
                epl6 = currentFrame.fieldOutputs['SDV161'].values

                nd = len(epl1)
                eg1 = 0.; eg2 = 0.; eg3 = 0.
                eg4 = 0.; eg5 = 0.; eg6 = 0.
                egeq = 0.
		#The SDV values are in abaqus notation as Ep11,Ep22,Ep33,Ep12,Ep13,Ep23 
		#The formula for calculation of equivalanat plastic strain requires the plastic tensor to be in voigt nottaion as 			# Ep11,Ep22,Ep33,2*Ep23,2*Ep13,2*Ep12

                for i in range(nd):
                    e1 = epl1[i].data
                    e2 = epl2[i].data
                    e3 = epl3[i].data
                    e4 = 2*epl4[i].data #2* was added
                    e5 = 2*epl5[i].data #2* was added
                    e6 = 2*epl6[i].data #2* was added
                    eg1 += e1
                    eg2 += e2
                    eg3 += e3
                    eg4 += e4
                    eg5 += e5
                    eg6 += e6
                    egeq += np.sqrt(2.*(e1**2 + e2**2 + e3**2 + 0.5*(e4**2 + e5**2 + e6**2))/3.)
                self.epl1.append(eg1/nd)
                self.epl2.append(eg2/nd)
                self.epl3.append(eg3/nd)
                self.epl4.append(eg4/nd)
                self.epl5.append(eg5/nd)
                self.epl6.append(eg6/nd)
                self.peeq.append(egeq/nd)
                ...
                for node in nodeObjectDict:
                    ...
                    displacement = displacementList.\
                        getSubset(region=nodeObjectDict[node].abqRegion).values
                    ...
                    ######
                    # Reaktion Force
                    reaktionForce = reaktionForceList.\
                        getSubset(region=nodeObjectDict[node].abqRegion).values
                    ...
                    ######
                    # Concentrated Force
                    concentratedForce = concentratedForceList.\
                        getSubset(region=nodeObjectDict[node].abqRegion).values
...
header_str = '# time (s), total strain [11, 22, 33, 12, 13, 23] (.), plastic strain [11, 22, 33, 12, 13, 23] (.), stress [11, 22, 33, 12, 13, 23] (MPa), equiv. total strain (.), equiv. plastic strain (.), equiv. stress (MPa)'
lt = len(Result1.temperature)
if lt > 0:
    header_str += ', Temperature (K)\n'
    out_val = zip(Result1.steptime,
              Result1.strain11Plot, Result1.strain22Plot, Result1.strain33Plot,
              Result1.strain12Plot, Result1.strain13Plot, Result1.strain23Plot,
              Result1.epl1, Result1.epl2, Result1.epl3,
              Result1.epl4, Result1.epl5, Result1.epl6,
              Result1.sigma11Plot, Result1.sigma22Plot, Result1.sigma33Plot,
              Result1.sigma12Plot, Result1.sigma23Plot, Result1.sigma13Plot,
              Result1.strainEPlot, Result1.peeq, Result1.sigmaVPlot,
              Result1.temperature)
else:
    header_str += '\n'
    out_val = zip(Result1.steptime,
