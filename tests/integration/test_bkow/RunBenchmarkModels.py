...
## Iso-hardening: Please deactivate the option for kinematic hardening in mod_wkcoup.f (i.e. Iwkcoup_bk = 0)
Cases.append('Ux')
Cases.append('Uy')
Cases.append('Uz') 
...
def writeFiles(Result, odb_trunk):
    ...
    with open(odb_trunk + '.csv','w') as f:
          f.write(header_str)
          np.savetxt(f, out_val, delimiter=',', fmt='%f')
            
def calculateError(data, data_ref):
    data = np.array(data)
    data_ref = np.array(data_ref)
    
    if np.shape(data)[1] == 3:
       err_strain = np.abs(data[:,1] - data_ref[:,1]) 
       err_stress = np.abs(data[:,2] - data_ref[:,2])
       out_val  = zip(err_strain, err_stress)
       header_str = 'plastic strain (.), stress  (MPa) \n'
    
    elif np.shape(data)[1] == 5:
       err_strain = np.abs(data[:,1:4] - data_ref[:,1:4])    
       err_stress = np.abs(data[:,4] - data_ref[:,4])
       out_val  = zip(err_strain[:,0], err_strain[:,1], err_strain[:,2] , err_stress)
       header_str = 'plastic strain [11, 22, 33] (.), stress (MPa) \n'
       
    norm_max = np.max(np.abs(np.array(out_val)[:,:]))
    return out_val, norm_max ,header_str

def deleteFiles(odb_trunk):
    extensions = ['.odb', '.msg', '.log', '.dat', '.com', '.sta', '.stt','.simlog', '.sim', 
                  '.prt', '.2.SMABulk', '.1.SMABulk', '.ipm', '_glob.csv']
...
for case in Cases:
    ...
    data     = np.loadtxt(case + '.csv', delimiter=',')
    data_ref = np.loadtxt(case + '_Ref.csv', delimiter=',')
    out_val, error_max ,header_str    = calculateError(data, data_ref)
    
    
    with open(odb_trunk + '_error.csv','w') as f:
        f.write(header_str)
        np.savetxt(f, out_val, delimiter=',')
    
    print('Maximum of error: ' + str(error_max))
    Message_file.write('Maximum_error of case ' + odb_trunk + ' :' + str(error_max) + '\n')
    # Delete abaqus files
